...
#
# This script builds a Conda environment based on the provided name or the default 'spherical-deepkriging'.
#
# Parameters:
#   -c, --conda_env       Specifies the name of the Conda environment (default: 'spherical-deepkriging')
#
# Examples:
#   1. Build the default Conda environment 'spherical-deepkriging':
#       ./build_conda_env.sh
#
...
CONDA_ENV="spherical-deepkriging"
export CC=$(which gcc)
export CXX=$(which g++)
...
build() {
    ...
    install_conda_packages
        # Check if build_cpp function exists before calling it
    if declare -F build_cpp > /dev/null; then
        echo -e "${FG_YELLOW}Building C++ extensions...${FG_RESET}"
        build_cpp
    else
        echo -e "${FG_RED}Warning: build_cpp function not found. Skipping C++ build.${FG_RESET}"
    fi
