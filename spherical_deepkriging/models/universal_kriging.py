...
class UniversalKriging:
    ...
    def compute_spherical_distance_matrix(coords_rad):
        ...
        cos_theta = np.sin(lat_rad[:, None]) * np.sin(lat_rad[None, :]) + np.cos(
            lat_rad[:, None]
        ) * np.cos(lat_rad[None, :]) * np.cos(lon_rad[:, None] - lon_rad[None, :])
    ...
    def fit(self, coords_train, phi_train, y_train, center_y=True):
        ...
        self._phi_train = (
            phi_train.astype(np.float32) if phi_train is not None else None
        )
        ...
        if not estimate_shape:
            try:
                ...
                if phi_train is None:
                    ...
                    self.gp_model.fit(
                        y=y_train_centered, X=self._phi_train, params=fit_params
                    )

                # Extract parameters
                nu_final, rho_final, sigma2_final, nugget_final = (
                    self.extract_gp_params(self.gp_model)
                )
                ...
                raise RuntimeError(
                    f"Model fitting failed with {self.cov_function} covariance: {str(e)}"
                )
        ...
        # Case 2: Estimate shape parameter (matern_auto)
            ...
            try:
                ...
                if phi_train is None:
                    ...
                    self.has_covariates = True
                    gp_model_estimate.fit(
                        y=y_train_centered, X=self._phi_train, params=fit_params
                    )

                # Extract estimated parameters
                nu_est, rho_est, sigma2_est, nugget_est = self.extract_gp_params(
                    gp_model_estimate
                )
    ...
    def predict(self, coords_new, phi_new=None, return_centered=True):
        ...
        if self.has_covariates:
            ...
            pred = self.gp_model.predict(
                X_pred=phi_new, gp_coords_pred=coords_rad, predict_var=False
            )
        else:
            pred = self.gp_model.predict(
                X_pred=None, gp_coords_pred=coords_rad, predict_var=False
            )
    ...
    def decompose_prediction(self, coords_new, phi_new):
        ...
        if len(coefs) == n_features:
            ...
            raise ValueError(
                f"Coefficient shape mismatch. Expected {n_features} or {n_features+1}, got {len(coefs)}"
            )
